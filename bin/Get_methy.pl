...
sub usage{
        die "perl $0  --mode <se|pe> Ref sam out.align new.sam\n" ;
}
...
#make a newsam for calling snp
	...
	if($out=~/\.gz$/){
		...
		print $pout "$line[0]\:rep$repeat\:$strand\:len$lenth\:score$line[4]\:$word\t$line[2]\t$line[3]\t$line[9]\t$methylation\t$refbase\t$line[10]\n";
	}
